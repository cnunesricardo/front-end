...
import { fetchProducts, fetchCategories } from 'src/app/utils/api.js'
...
const AdminEditProductsIDPage = ({ product }) => {
  ...
  useEffect(() => {
    ...
    const dropBox2 = ({ apiUrlCategories }) => {
...
const handleSubmit = async (event) => {
  ...
  const product_title = event.target.title.value;
  const product_price = event.target.price.value;
  const product_description = event.target.description.value;
  const product_image = event.target.image.value;
  const product_rate = '0'
  const product_count = '0' 
  const category_id = event.target.category.value;
  ...
  const data = {
    product_title, product_price, product_description, product_image, product_rate, product_count, category_id
  };
      ...
      <div className="mt-2 container mx-auto max-w-screen-lg flex justify-center flex-col items-center md:flex-row">
        ...
        <h1 className='container mx-auto pt-7 grid grid-cols-1 gap-2 place-items-center max-w-screen-lg text-2xl font-bold underline'>
         ...
         <form className="container mx-auto max-w-screen-lg flex justify-center grid grid-cols-1 gap-2 place-content-center" onSubmit={handleSubmit}>
            ...
            <select className="text-black ring-2 rounded-lg ring-zinc-950 w-60" id="category" name="category" type="text" required
